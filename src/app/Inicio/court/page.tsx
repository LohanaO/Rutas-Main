...
export default function Court() {
  ...
  const [currentPageVendidos, setCurrentPageVendidos] = useState(1);
  const [currentPageNoVendidos, setCurrentPageNoVendidos] = useState(1);

  // Función para obtener productos paginados
  const getPaginatedProducts = (products: Producto[], currentPage: number) => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return products.slice(startIndex, endIndex);
  };

  // Obtener productos paginados para productos vendidos y no vendidos
  const paginatedVendidos = getPaginatedProducts(
    currentCourt.productosVendidos,
    currentPageVendidos
  );
  const paginatedNoVendidos = getPaginatedProducts(
   currentCourt.productosNoVendidos,
    currentPageNoVendidos
   
  );

  const handlePageChangeVendidos = (selectedPage: number) => {
   
    setCurrentPageVendidos(selectedPage);
  };

  const handlePageChangeNoVendidos = (selectedPage: number) => {
    setCurrentPageNoVendidos(selectedPage);
  };
      ...
      >
        ...
        {showPDF ? (
          ...
          <div className="xl:grid w-full flex-1 xl:grid-cols-3 xl:grid-rows-2  transition duration-300  ">
            <button
              className="xl:hidden flex bg-[#ccc] p-1 absolute top-0 right-2 m-5 rounded-lg items-center cursor-pointer  transition duration-300 transform hover:scale-110"
            ...
            */
            ...
            <div className="flex text-black px-3 xl:col-span-2 col-span-3 col-start-1 row-start-1 ">
              <TableProduct
                products={paginatedVendidos}
                totalItems={courtResponse.productosVendidos.length}
                itemsPerPage={itemsPerPage}
                onPageChange={handlePageChangeVendidos}
              />
            </div>
            {/* Tercer div */}
            <div className="flex text-black px-3 xl:col-span-2 col-span-3 row-start-2">
              <TableProduct
                products={paginatedNoVendidos}
                totalItems={courtResponse.productosNoVendidos.length}
                itemsPerPage={itemsPerPage}
                onPageChange={handlePageChangeNoVendidos}
              />
            </div>
            {/* Segundo div */}
